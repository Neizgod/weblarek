...
`modal:open` - генерирует событие открытия модального окна.

`modal:close` - генерирует событие закрытия модального окна.

`card:select` - генерирует события выбора карточки.

`card:renderPrewiew` - генерирует событие для отображения превью карточки.

`card:addToBasket&removeFromBasket` - генерирует событие для добавления товара в корзину, изменения счетчика корзины в header !!!! ДОДЕЛАТЬ
