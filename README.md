...
Методы:
`set title(title: string):` - установка названия товара;
`set price(price: number | null)` - установка цены товара;
...
`constructor(container: HTMLElement, action?: IFormAction)` - принимает ссылку на DOM элемент за отображение которого он отвечаети и объект с методами для обработки события.

Поля класса:
`errorsElement: HTMLElement` - ссылка на элемент для отображения ошибок;
`buttonElement: HTMLButtonElement` - ссылка на элемент кнопки формы;

Методы:
`set textErrors(error: string)` - установка текста ошибки формы. 
...
Поля класса:
`paymentOnlineButton: HTMLButtonElement` - хранит ссылку на кнопку выбора оплаты онлайн способом.
`paymentCashButton: HTMLButtonElement` - хранит ссылку на кнопку выбора оплаты наличным способом.
`addressElement: HTMLElement` - хранит ссылку на пользовательнский ввод адреса доставки.
...
Поля класса:
`emailElement: HTMLElement` - хранит ссылку на пользовательнский ввод email.
`phoneElement: HTMLElement` - хранит ссылку на пользовательнский ввод телефона.
...
Поля класса:
`modalContainer: HTMLElement` - ссылка на DOM элемент отображения контента.
`closeButton: HTMLButtonElement`  - ссылка на DOM кнопки закрытия модального окна.

Методы:
`set content(value: HTMLElement[])` - установка элементов которые будут выведены в модальном окне.
...
Поля класса:
`containerBasket: HTMLElement` - ссылка на элемент, который будет хранить в себе товары.
`totalElement: HTMLElement` - ссылка на элемент который отображает всю сумму в корзине.
`basketButton: HTMLButtonElement` - ссылка на кнопку корзины.

Методы:
`set content(value: IProduct[])` - установка элементов которые будут выведены в корзине.
`set total(value: number)` - установка общей суммы товаров в корзине.
...
Поля класса:
`totalElement: HTMLElement` - ссылка на элемент отображающий общую сумму покупки.
`successButton: HTMLButtonElement` - ссылка на элемент кнопки после удачной покупки.

Методы:
`set total(value: number)` - установка общей суммы покупки.

##### Презентер

Весь код данного слоя написан в main.ts.

##### События

`catalog:changed` - генерирует событие изменения каталога товаров.
Обработчик: загружает карточки товара в галерею.

`basket:open` - генерирует событие открытия корзины товаров.
Обработчик: открывает модальное окно с корзиной.

`modal:close` - генерирует событие в связи с закрытием модального окна.
Обработчик: закрывает модальное окно.

`card:select` - генерирует событие в связи с выбором карточки.
Обработчик: устанавливает текущий товар в классе CatalogProducts.

`catalog:changedCurrentProduct` - генерирует событие в связи с изменением текущего товара в классе CatalogProducts.
Обработчик: открывает модальное окно с карточкой текущего товара.

`basket:changedContent` - генерирует событие в связи с изменением содержимого корзины.
Обработчик: изменяет счетчик в Header, осуществляет перерендер содержимого корзины.

`basket:deleteItem` - генерирует событие в связи с удалением товара из корзины.
Обработчик: удаляет товар из корзины.

`basket:deleteItemFromPreview` - генерирует событие в связи с удалением товара из корзины из cardPreview.
Обработчик: удаляет товар из корзины и закрывает модальное окно.

`card:addToBasket` - генерирует событие в связи с добавлением товара в корзину.
Обработчик: добавляет товар в корзину и закрывает модальное окно.

`form: openOrder` - генерирует событие в связи с открытием формы с заказом.
Обработчик: открывает модальное окно с формой Order.

`selectPayment` - генерирует событие в связи с выбором способа оплаты.
Обработчик: устанавливает способ оплаты в customerData.

`customerData: paymentOrAddressChanged` - генерирует событие в связи с изменением выбора оплаты или адресса.
Обработчик: рендерит форму formOrder с ошибками.

`form:changeAddress` - генерирует событие в связи с вводом адресса.
Обработчик: устанавливает адресс в customerData.

`form: openContacts` - генерирует событие в связи с открытием formContacts.
Обработчик: рендерит formContacts.

`form:changeEmail` - генерирует событие в связи с вводом email.
Обработчик: устанавливает email в customerData.

`form:changePhone` - генерирует событие в связи с вводом телефона.
Обработчик: устанавливает телефон в customerData.

`customerData: emailOrPhoneChanged` - генерирует событие в связи с изменением email или телефона.
Обработчик: рендерит форму formContacts с ошибками.

`form: startShopping` - генерирует событие в связи оформлением заказа.
Обработчик: отправляет запрос на сервер с введенными данными и id выбранных товаров, очишает данные в корзине и customerData, очищает формы вызывает модальное окно с информированием об успешной покупке.